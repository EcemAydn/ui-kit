...
const headers = ref([
  ...
  { name: "HİZMET", value: "service_titles" },
...
<template>
  <div class="w-full overflow-y-auto pb-24">
    <HeaderComp title="All users">
      <template #actions>
        <div class="flex gap-2">
          <ButtonComp button-name="Yeni Personel" color="secondary">
            <template #append>
              <Plus class="w-4 h-4" />
            </template>
          </ButtonComp>
    ...
    <TableComp selectable :headers="headers" :items="appointmentStore.staffs">
      ...
      <template #service_titles="{ item }">
        <div class="flex items-center gap-2">
          <div>
            <p>
              {{ item.service_titles[0] }}
            </p>
          </div>
        </div>
      </template>
